...
export async function init(router) {
    router.get("/search", async (req, res) => {
        try {
            ...
            if (result) {
                ...
                console.error(
                    `[G-Player Proxy] ${source} 所有搜索 API 都失败了`,
                );
                res.status(500).json({
            ...
            console.error("[G-Player Proxy] 搜索失败:", error.message);
            res.status(500).json({
                error: "Search failed",
    ...
    router.get("/song", async (req, res) => {
        try {
            ...
            if (result) {
                ...
                console.error(
                    `[G-Player Proxy] ${source} 所有歌曲 API 都失败了`,
                );
                res.status(500).json({ error: "All song APIs failed" });
            }
        } catch (error) {
            console.error("[G-Player Proxy] 获取歌曲失败:", error.message);
            res.status(500).json({
    ...
    router.get("/lyric", async (req, res) => {
        try {
            ...
            switch (source) {
                case "netease":
                    result = await tryMultipleAPIs([
                        ...
                        {
                            name: "openmusic-netease-lyric-fallback",
                            url: `${API_CONFIG.OPEN_MUSIC_API_URL}?provider=netease&type=lyric&id=${id}&token=${API_CONFIG.OPEN_MUSIC_API_TOKEN}`,
                            transform: (data) => {
                                if (data?.code === 200 && data?.data) {
                    ...
                    result = await tryMultipleAPIs([
                        {
                            ...
                            url: `${API_CONFIG.OPEN_MUSIC_API_URL}?provider=kw&type=lyr&id=${id}&format=all&token=${API_CONFIG.OPEN_MUSIC_API_TOKEN}`,
                            transform: (data) => {
                                if (data?.code === 200 && data?.data) {
                                    let lrcContent = "";
                                    if (
                ...
                default:
                    result = await tryMultipleAPIs([
                        ...
                        {
                            name: "openmusic-tencent-lyric-fallback",
                            url: `${API_CONFIG.OPEN_MUSIC_API_URL}?provider=qq&type=lyric&id=${id}&token=${API_CONFIG.OPEN_MUSIC_API_TOKEN}`,
                            transform: (data) => {
                                if (data?.code === 200 && data?.data) {
                                    const lyricData = data.data;
                                    if (lyricData.lyric || lyricData.lrc) {
                                        return {
                                            data: {
                                                lrc:
                                                    lyricData.lyric ||
            ...
            console.error("[G-Player Proxy] 获取歌词失败:", error.message);
            res.json({ data: { lrc: "", tlyric: "", trans: "" } });
        }
    });
    ...
    router.get("/stream", async (req, res) => {
        try {
            ...
            console.error("[G-Player Proxy] 音频流失败:", error.message);
